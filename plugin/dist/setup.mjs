...
async function main() {
  ...
  if (lD(optOut)) return ue("Cancelled.");
  if (optOut) {
    try {
      await writeFile2(
        join2(cwd, IGNORE_MARKER),
    ...
    cfg.optOutProjects = cfg.optOutProjects.filter((d3) => d3 !== cwd);
  }
  ...
  $e(import_picocolors3.default.green("Setup complete \u2014 your sessions now sync automatically (except opted-out projects)."));
}
main().catch((err) => {
  console.error(err);
  process.exit(1);
