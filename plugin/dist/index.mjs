...
var SERVER = process.env.CLAUDELENS_SERVER ?? "http://localhost:4000";
var TOKEN = process.env.CLAUDELENS_TOKEN ?? "";
async function collectSessions() {
  ...
  for (const dir of projectDirs) {
    const full = join2(PROJECTS_DIR, dir);
    let files;
    try {
...
async function main() {
  console.clear();
  oe(import_picocolors3.default.bgCyan(import_picocolors3.default.black(" ClaudeLens ")) + import_picocolors3.default.dim(" share a session so your team can learn"));
  const candidates = await collectSessions();
  if (!candidates.length) {
    ue(`No Claude Code sessions found in ${PROJECTS_DIR}`);
    process.exit(1);
  }
  const picked = await ie({
    message: `Pick a session to share  ${import_picocolors3.default.dim("(server: " + SERVER + ")")}`,
  ...
  le(
    [
      `${import_picocolors3.default.bold("Title")}    ${s.title}`,
      `${import_picocolors3.default.bold("Project")}  ${s.project ?? "\u2014"}  (branch: ${s.gitBranch ?? "\u2014"})`,
      `${import_picocolors3.default.bold("Turns")}    ${s.stats.turns}  \xB7  models: ${s.stats.models.join(", ") || "\u2014"}`,
      `${import_picocolors3.default.bold("Tokens")}   ${s.stats.totalTokens.toLocaleString()}  (~${fmtCost(
        s.stats.estimatedCostUsd
      )})`,
      `${import_picocolors3.default.bold("Skills")}   ${s.stats.skills.join(", ") || "\u2014"}`,
      `${import_picocolors3.default.bold("Agents")}   ${s.stats.subagents.join(", ") || "\u2014"}`,
      hitEntries.length ? import_picocolors3.default.yellow(
        `Redacted secrets: ${hitEntries.map(([k2, v2]) => `${k2}\xD7${v2}`).join(", ")}`
      ) : import_picocolors3.default.green("No secrets detected by the redactor.")
    ].join("\n"),
    "Preview"
  );
  ...
  try {
    ...
    spin.stop("Uploaded.");
    $e(import_picocolors3.default.green(`\u2714 Shared! ${SERVER.replace(/:\d+$/, ":5173")}/session/${data.id}`));
  } catch (err) {
